use linera_alloy_json_abi::{Error, JsonAbi, Param};
...
fn complex_error() {
    ...
    assert_eq!(
        ...
        Error {
            inputs: vec![Param {
                internal_type: Some(linera_alloy_json_abi::InternalType::Other {
                    contract: None,
                    ty: "string".into()
                }),
                name: "reason".into(),
...
fn big_function() {
    ...
    let f = serde_json::from_str::<linera_alloy_json_abi::Function>(s).unwrap();
    assert_eq!(f.signature(), expected);
    assert_eq!(f.selector(), linera_alloy_primitives::keccak256(expected)[..4]);
    assert_eq!(f.selector(), linera_alloy_primitives::hex!("87201b41"));
